...
All shared test helpers live in `crates/test-support/src/lib.rs`. Every consumer has `test-support = { path = "../test-support" }` as a dev-dependency, so `use test_support::...` works from workspace crate tests such as `crates/reef-app/tests/*.rs`, `crates/reef-io/tests/*.rs`, and `crates/reef-tui/tests/*.rs`.
...
For pure core logic (e.g. `tree::build` in `crates/reef-core/src/git/tree.rs`), assert on the returned data structure directly — don't route through rendering.
...
- `crates/reef-core/tests/git_graph_properties.rs` — the proptest version, which generates topologically-ordered commit vectors from a random shape
