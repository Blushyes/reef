...
use crate::text::{Rgb, StyledToken, TextStyle};
...
fn highlight_lines(
    ...
    for line in lines {
        let tokens = match h.highlight_line(line, &SYNTAX_SET) {
            Ok(ranges) => ranges
                .into_iter()
                .map(|(s, text)| StyledToken::new(convert_style(s), text))
                .collect(),
            Err(_) => vec![StyledToken::new(TextStyle::default(), line)],
        };
        out.push(tokens);
    }
    out
...
fn convert_style(s: syntect::highlighting::Style) -> TextStyle {
    TextStyle {
        ...
        bold: s.font_style.contains(FontStyle::BOLD),
        italic: s.font_style.contains(FontStyle::ITALIC),
        underlined: s.font_style.contains(FontStyle::UNDERLINE),
    }
}
