...
- Renderer hosts dispatch preview definition requests with a typed source cursor and viewport budget. `reef-app` marks the initial workspace index stale, schedules its build from `step`, and owns enrichment- and workspace-index-bound retry, definition/reference resolution, candidate state, selected-candidate preview, history, and committed jumps; hosts own only coordinate mapping and popup geometry.
